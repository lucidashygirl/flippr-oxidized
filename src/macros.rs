macro_rules! collision_generator {
    ($name:tt, $type:ty, $component:expr) => {
        ...
        pub fn $name(
            ...
            wall_query: Query<(&GridCoords, &ChildOf), Added<$type>>,
            parent_query: Query<&ChildOf, Without<$type>>,
            ...
            level_query.iter().for_each(|(level_entity, level_iid)| {
                let level_walls = match level_to_wall_locations.get(&level_entity) {
                    Some(l) => l,
                    None => return,
                };
